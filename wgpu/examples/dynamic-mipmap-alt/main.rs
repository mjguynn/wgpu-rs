...
struct Example {
    ...
    read_future: Option<Box<dyn Unpin + Future<Output = Result<(), wgpu::BufferAsyncError>>>>,
...
impl Example {
    ...
    fn submit_mipmap_read(
        ...
        wgpu::Buffer,
        impl Unpin + Future<Output = Result<(), wgpu::BufferAsyncError>>,
    ) {
        ...
        Self::generate_mipmaps(&mut encoder, device, texture, MIP_LEVEL_COUNT);
...
impl framework::Example for Example {
    ...
    fn init(
        ...
        let (read_buf, read_future) =
            Example::submit_mipmap_read(device, queue, &random_texture_handle, encoder);
        ...
        Example {
            ...
            draw_pipeline,
            read_buf,
            read_future: Some(Box::new(read_future)),
            random_texture,
    ...
    fn render(
        ...
        pollster::block_on(self.read_future.take().unwrap()).unwrap();
        let clear_color = {
            let data = self.read_buf.slice(..).get_mapped_range();
        ...
        let (read_buf, read_future) =
            Example::submit_mipmap_read(device, queue, &self.random_texture_handle, encoder);
        self.read_buf = read_buf;
        self.read_future = Some(Box::new(read_future));
    }
}
